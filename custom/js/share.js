...
// (function () {
    //设置分享
    function share(id,title,img,desc){
        try {
            //是微信的情况下设置分享
            if ((window.navigator.userAgent.toLowerCase()).match(/MicroMessenger/i) == 'micromessenger'){
                ...
                $.ajax({
                    url: "/healthCloud-open-gateway/gateway/getWechatJsapiSignature",
                    type: "GET",
                    cache: false,
                    data:{ url:fullUrl },
                    ...
                    success: function (data) {
                        if (data.code == 0) {
                            ...
                            wx.config({
                                ...
                                jsApiList: ['getLocation', 'openLocation','showMenuItems','onMenuShareAppMessage','onMenuShareTimeline','chooseImage','uploadImage'] // 必填，需要使用的JS接口列表
                            });
                            wx.ready(function () {
                                //分享到朋友圈
                                wx.onMenuShareTimeline({
                                    title: title, // 分享标题
                                    link:shareLinkHost+"html/jump.html?id="+id+"&isShare=1", // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                                    imgUrl: img, // 分享图标
                                    success: function () {
                                        // 用户确认分享后执行的回调函数
                                    }
                                ...
                                wx.showMenuItems({
                                    menuList: ["menuItem:share:appMessage","menuItem:share:timeline","menuItem:favorite","menuItem:copyUrl","menuItem:profile","menuItem:addContact"], // 要显示的菜单项，所有menu项见附录3
                                    success:function () {
                                        console.log( 'yes' );
                                    },
                                    fail:function (res) {
                                ...
                                wx.onMenuShareAppMessage({
                                    title: title, // 分享标题
                                    desc: desc, // 分享描述
                                    link:shareLinkHost+"html/jump.html?id="+id+"&isShare=1",
                                    imgUrl: img, // 分享图标
